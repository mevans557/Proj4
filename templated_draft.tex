...
Percolation is a long-researched process, largely focussing on giant-component formation \citep{weissteinBondPercolation}, \citep{duminil-copinSixtyYearsPercolation2017}. Here I refer to a percolation-type graph as one constructed according to the bond-percolation process, where a lattice is reduced such that each edge has an independent probability $p$ of existing. Much work has been done on finding the critical probabilities, $p_c$ associated with various lattices. These are the probabilities at which giant components are formed, where for values of $p < p_c$, giant components have a $0$ probabilty of forming (on an infinite lattice), and for $p > p_c$ giant component formation occurs almost surely. Bond-percolation is a less general case than site-percolation, however exploring all different kinds of percolation, on different base lattices, would be beyond the scope of this project. In \citet{browneInfectionPercolationDynamic2021}, the authors discoverd that a site-based model of disease spread demonstrated cricicality effects very similar to that of site-percolation. They also demonstrated how the critical threshold for percolation impacted the overall critical threshold for disease to spread throughout the population, and how the recovery time affected the critical infectivity. However, one limitation of this model was the choice to use a fixed recovery time - though this in reality would follow some kind of distribution, as the underlying process is a Markov jump process.

\section{Project Scope}

We seek here to expand on this previous work, by creating a stochastic model of disease spread on a random (percolation-type) network, and observing how changes both to the network structure and to the disease parameters qualitatively affect the epidemic. These results are compared to stochastic simulations using the Gillespie SSA. Where interesting differences are found, we explore these in more depth. We calculate, in different cases, the mean times for different parts of the process, such as recovery and the transition in SEIR models from Exposed to Infectious, and compare these to simulation results. We explore the effect of the graph structure, parameterised by the edge probability $p$, on epidemic extent and speed. We also explore a variety of qualitative effects, arising as a result of the graph structure.

\chapter{The SI Model}

\section{Generating the Adjacency Matrix}

Much of this was inspired by some comments on a stack overflow thread \citep{guillefixAnswerHowCan2017}, although the explanation given in the discussion was very terse, and not clear exactly how the method suggested worked. We expand on this and explain where the different stages in the process come from.
...
Next, a graph-based implementation of the Gillespie algorithm for the simple SI model was made. This was done by interpreting the SI model as the following reaction:
...
\begin{enumerate}
	...
	\end{equation}
	Where $I_{pos, i}$ is the $i$th element of $\v{I}_{pos}$, and $k_I$ is the infection rate, interpreted as the probability that a given connection between an infected and susceptible node results in the infection spreading from the infected node to the susceptible node in a given time step. Thus every such connection has an equal chance of spreading the infection, and node degree has no bearing on the probability of a given edge spreading the infection.
...
The next most simple model is the SIS model, which is an accurate portrayal of various STIs, wherein an individual can be infected by another infected individual, and can later become uninfected, but they do not ever become immune to the disease. This is represented by the systems

\begin{equation}
	\ce{S + I ->[k_I] 2I} \qquad \ce{I ->[k_U] S}.
\end{equation}

Here, $k_I$ is still the infection rate, and $k_U$ is the rate of recovery without immunity. Once again, we implemented a well-mixed Gillespie SSA as well as the graph-based models. This was again on a square lattice with side length $100$, with $k_I = 0.5$ and $k_U = 0.3$ for the graph-based model. The parameters in the well-mixed model were scaled appropriately to give comparable results on the same population.
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/graphSIS0.3.png}
		\caption{$p = 0.3$}
		\label{fig:SIS3}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/graphSIS0.5.png}
		\caption{$p = 0.5$}
		\label{fig:SIS5}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/graphSIS0.7.png}
		\caption{$p = 0.7$}
		\label{fig:SIS7}
	\end{subfigure}
	\caption{Graph SIS for different $p$}
	\label{fig:GraphSIS}
\end{figure}
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/graphSIS0.3end.png}
		\caption{$p = 0.3$}
		\label{fig:SIS3end}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/graphSIS0.5end.png}
		\caption{$p = 0.5$}
		\label{fig:SIS5end}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/graphSIS0.7end.png}
		\caption{$p = 0.7$}
		\label{fig:SIS7end}
	\end{subfigure}
	\caption{Graph SIS for different $p$, end state}
	\label{fig:GraphSISends}
\end{figure}

\subsection{Analysis}

Here we begin to see some strong qualitative differences between the well-mixed and graph-based simulations, as well as striking differences between critical, subcritical, and supercritical graphs. For the sub-critical graph ($p = 0.3$), the low level of connectivity leads most infection outbreaks to die out very rapidly. Only a few outbreaks last for a significant amount of time, and these experience large relative fluctuations in the number of infected individuals. The extent of the epidemic is also much lower, as we expect from the small connected components of such a graph. In the critical ($p = 0.5$) case, the infected populations follow an almost linear trajectory, with the exception of the scenarios where the infection happens to begin in a small connected component of the graph, and thus very quickly becomes endemic in that region, and does not spread beyond it. The supercritical ($p = 0.7$) case is far closer to the well-mixed model, and roughly follows the logistic-like curves that we also see in the SI model. Looking at the graph plots at the end of various simulation runs, the overall shapes seem very similar, though they appear somewhat ``holey'' or porous. This is as a result of some of the infected population becoming susceptible again.
